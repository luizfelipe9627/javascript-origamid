// Função chamada outsideClick responsável por fazer o dropdown sumir ao clicar fora dele, recebe três parâmetros, um sendo o element(data-dropdown), o outro sendo os eventos e o outro o callback(transformada em uma função).
...
export default function outsideClick(element, events, callback) {
  ...
  if (!element.hasAttribute(outside)) {
    ...
    events.forEach((userEvent) => {
      html.addEventListener(userEvent, handleOutsideClick); // Está adicionando um evento de clique no HTML, que ao ser acionado executa a função handleOutside.
    });
